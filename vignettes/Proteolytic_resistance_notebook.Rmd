...
author: Valentina Cappelletti (<cappelletti@imsb.biol.ethz.ch>),Malinovska Liliana
  (<malinovska@imsb.biol.ethz.ch>)
output:
  pdf_document:
    toc: yes
  html_notebook:
...
TODO: Change variable names to match the included data name (don't run these for vignette output)
...
```{r Adjust aSynuclein nomenclature to match fasta file, eval=FALSE, echo=TRUE, message=FALSE, warning=FALSE, include=TRUE}
...
results = calculateProteolyticResistance(MSstatsLiP_Summarized, fasta_file, differential_analysis = TRUE)
results$RunLevelData
results$groupComparison

ResistanceBarcodePlotLiP(results,
                           fasta_file,
                           which.prot = "P16622",
                           which.condition = "F1",
...
StructuralBarcodePlotLiP(results$groupComparison, 
...
Accessibility <-  MSstatsLiP_Summarized[["LiP"]]$ProteinLevelData %>% 
        select(FULL_PEPTIDE, Protein, LogIntensities, GROUP,SUBJECT) %>% 
    inner_join(MSstatsLiP_Summarized[["TrP"]]$ProteinLevelData %>% 
        select(Protein, LogIntensities, GROUP, SUBJECT ),by = c('Protein','GROUP','SUBJECT'), suffix = c('_LiP', '_TrP')) %>% 
        group_by(Protein, SUBJECT) %>% 
        #mutate(LiP_norm =  2 ^ (LogIntensities_LiP-LogIntensities_TrP))%>%
        mutate(LiP_norm = 2 ^ (LogIntensities_LiP-LogIntensities_TrP))%>%
        mutate(LiP_norm_SD = sd(2 ^ (mean(LogIntensities_LiP)-mean(LogIntensities_TrP))))%>%
        #ungroup()%>%
        group_by(Protein, GROUP) %>% 
        mutate(Accessibility_ratio = (mean(LiP_norm)))%>%
        mutate(Accessibility_ratio_SD = sd(LiP_norm))%>%
        left_join(FullyTrP, by=c("Protein"="ProteinName2"))
```

```{r load tripticity data, echo=TRUE, message=FALSE, warning=FALSE,include=TRUE, eval=FALSE}

tripticity <- read_delim(file=choose.files(caption="Choose dataset with trypticity information"), 
                          delim=",", escape_double = FALSE, trim_ws = TRUE)
```


```{r filter peptides with missed cleavages, echo=TRUE, message=FALSE, warning=FALSE,include=TRUE}
tripticity<- tripticity%>%filter(hasMiSSedCl==FALSE)
Accessibility_full <- Accessibility[Accessibility$PeptideSequence %in% tripticity$peptide, ]
```
...
Accessibility_full%>% mutate(Protease_resistance=case_when(Accessibility_ratio>1 ~1,TRUE~Accessibility_ratio))%>%
  filter(ProteinName=="P37840")%>%
  ggplot()+
  geom_rect(aes(ymin=61, ymax=95, xmin=0.85, xmax=1.15), fill = NA,color=1)+
  geom_vline(xintercept = 1, size=20, color="black")+
  geom_linerange(aes(x=1, ymin=StartPos, ymax=EndPos,colour=Protease_resistance), size=20)+
  coord_flip()+
  ...
  guides()+
  facet_grid(~GROUP)+
  labs(y="aSynuclein sequence [AA]",x="")+
  ylim(0,140)+
...
full <- inner_join(
    MSstatsLiP_model[["LiP.Model"]] %>% 
        ...
        select(ProteinName, PeptideSequence, Label, log2FC, SE, DF),
    MSstatsLiP_model[["TrP.Model"]] %>% 
        filter(abs(log2FC) != Inf) %>% 
        select(ProteinName = Protein, Label, log2FC, SE, DF) %>% 
        rename(log2FC_ref = log2FC, SE_ref = SE, DF_ref = DF)
)
...
FullyTrp.Model <- data.table(
    Protein = str_extract(full$ProteinName, regex_uniprot_iso),
    PeptideSequence = full$PeptideSequence,
    Label = full$Label,
    log2FC = log2fc,
    SE = stderr,
    Tvalue = tval,
    DF = df,
    pvalue = pval,
    adj.pvalue = p.adjust(pval, method = 'BH')
)

```
Inspect `FullyTrp_model`. 
```{r Inspect protection model, message=FALSE, warning=FALSE, echo=TRUE,include=TRUE}
head(FullyTrp.Model)
```
...
```{r Select aSynuclein, message=FALSE, warning=FALSE, echo=TRUE,include=TRUE}

P37840=FullyTrp.Model%>%filter(Protein=="P37840"&!Label%in%c("F1 vs F2","M1 vs M2"))
```

```{r add position of peptides, fig.height=5, fig.width=15, echo=TRUE, include=TRUE}
P37840 <- P37840 %>% left_join(FullyTrP, by=c("Protein"="ProteinName","PeptideSequence"="PeptideSequence"))

```

```{r set cutoff values, fig.height=5, fig.width=15, echo=TRUE, include=TRUE}
...
P37840%>%
  mutate(significant=case_when(log2FC>log2FC.cutoff&adj.pvalue<adj.pvalue.cutoff~"Yes,UP",
                               log2FC< -log2FC.cutoff&adj.pvalue<adj.pvalue.cutoff~"Yes,DOWN",TRUE~"No")) %>%
  print() %>%
  ggplot()+
  geom_rect(aes(ymin=61, ymax=95, xmin=0.85, xmax=1.15), fill = NA,color=1)+
  geom_vline(xintercept = 1, size=20, color="black")+
  geom_linerange(aes(x=1, ymin=StartPos, ymax=EndPos, color=significant), size=20)+
  gghighlight((significant=="Yes,UP" | significant=="Yes,DOWN"), calculate_per_facet = TRUE, use_direct_label = FALSE, use_group_by = FALSE, unhighlighted_params = list(colour =NULL))+ 
  coord_flip()+
  scale_color_manual(values = c("Yes,UP"="darkred", "Yes,DOWN"="blue"))+
  theme_classic()+
  guides(color="none")+
  facet_grid(~Label)+
  labs(y="aSynuclein sequence [AA]",x="")+
