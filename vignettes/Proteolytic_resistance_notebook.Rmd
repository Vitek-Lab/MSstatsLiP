...
```{r load TrP data, echo=TRUE, message=FALSE, warning=FALSE, include=TRUE, eval = FALSE}

raw_prot <- read_delim(file=choose.files(caption="Choose TrP dataset"), 
                          delim=",", escape_double = FALSE, trim_ws = TRUE)
```

```{r Adjust aSynuclein nomenclature to match fasta file, echo=TRUE, message=FALSE, warning=FALSE,include=TRUE}
...
FullyTrP <- msstats_data[["LiP"]] %>% 
  ...
  mutate(ProteinName2 = str_c(ProteinName, 1:n(), sep = "**")) %>% 
  ungroup()
```
...
msstats_data[["LiP"]] <- msstats_data[["LiP"]] %>% 
  select(-ProteinName) %>% 
  inner_join(FullyTrP %>% select(ProteinName = ProteinName2, PeptideSequence, StartPos, EndPos))
```
